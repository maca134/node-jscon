...
Rcon.prototype.kick = function (player, reason) {
	reason = reason || '';
	this.beclient.sendCommand(util.format('kick %s %s', player, reason));
};

Rcon.prototype.ban = function (player, reason) {
	reason = reason || '';
	this.beclient.sendCommand(util.format('ban %s %s', player, reason));
};
...
Rcon.prototype._message = function (message) {
	var playerconnect = message.match(/^Player #([0-9]+) (.*) \(([0-9\.]+):[0-9]+\) connected$/);
	if (playerconnect) {
		var row = {
			id: playerconnect[1],
