...
function Monitor(rcon) {
	...
	var self = this;
	this.rcon = rcon;
	this.rcon.on('playerconnect', data => logger(util.format('%s (%s) connected', data.name, data.ip)), 'connected');
	this.rcon.on('playerdisconnect', data => logger(util.format('%s disconnected', data.name)), 'disconnected');
