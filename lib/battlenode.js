            ...
            switch (payload.readUInt8(1)) {
                case BE_LOGIN_PACKET:
                    ...
                    if (loggedIn == true) {
                        ...
                        this.emit('login', 'invalid password');
                    }
                    break;
...
BattleNode.prototype.sendCommand = function(command, callback) {
    logger(util.format('Sending command: %s', command), 'debug');
    var self = this;
...
BattleNode.prototype.disconnect = function(err) {
    clearInterval(this.keepalive);
    logger(util.format('rcon disconnected: %s', err));
